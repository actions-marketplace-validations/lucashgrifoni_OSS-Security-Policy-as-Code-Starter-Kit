...
The honest answer is below. This page deliberately does not market the kit; it draws boundaries. Everything described here as **shipped** is verified against the current released build; see `CHANGELOG.md` for the authoritative version and counts. The items formerly listed at the end under *Roadmap (v6.0.0 — in development)* have since shipped across the v6.0.0–v6.4.0 line; that section is retained as historical context.
...
- **222 controls** each labelled `deterministic` (clone-truth, no inference), `signal` (directional, not verified), or `evidence-backed` (consumes a structured evidence file with a schema). The grade flows into the `reports/2.0` JSON and Markdown output so consumers can reason about proof strength.
...
- **AIBOM generator or AI conformity engine.** Since v6.0.0 the kit ships an AIBOM **presence** check (`AIBOM-PRESENT-001`) and LLM-oriented advisory profiles (`appsec-llm-ssdf-218a-1` for NIST SP 800-218A, `cra-eu-ai-act-art11-1` for EU AI Act Article 11 + Annex IV). It still does **not** generate an AIBOM for the target, nor does it certify AI-Act / SSDF conformance — the profiles emit alignment evidence only.
...
| **Evidence store** | [Chainloop](https://chainloop.dev/), [GUAC](https://guac.sh/) | Server-side aggregation + query of attestations | Ships an experimental Chainloop exporter (`export-evidence --format chainloop`); a GUAC exporter is not yet available. The kit does not itself aggregate or persist evidence server-side |
...
| **What you bring** | The entire ruleset — you write and maintain every policy | The policies — you write ClusterPolicies / validating rules | Almost nothing: 222 pre-authored controls + 56 composable profiles ship in the box |
...
- **Use OPA/Conftest** when you need arbitrary policy over arbitrary config (Terraform plan gating, Kubernetes manifests, custom JSON contracts) and you are willing to own the ruleset. The kit does **not** replace it, but it does ship a **CEL/Rego export** today via `export-policy --format {rego,cel}` (ADR-035), which renders an OPA/Rego or CEL skeleton over a `reports/2.0` report so its verdicts can feed an OPA-based gate.
