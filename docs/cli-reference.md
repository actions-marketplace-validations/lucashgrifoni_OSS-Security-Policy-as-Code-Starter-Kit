...
| `evaluate` | `--profile/-p` (or positional target) | `--target/-t`, `--output-dir/-o`, `--format/-f`, `--summary-only/-so`, `--fail-on/-fo` (`none`/`fail`/`degraded`), `--waivers/-w`, `--scorecard-json/-sj`, `--report-json-contract` (`2.0`/`1.0`/`0.3`/`0.2`), `--use-insights-evidence`, `--sarif-output`, `--include-absolute-path` (opt-in; default is privacy-by-default basename), `--verbose/-v`, `--quiet/-q`, `--kit-root/-k` | Default contract is `2.0` (v7.0.0 flip, ADR-027); `1.0`/`0.3`/`0.2` remain selectable. `--use-insights-evidence` (default off) lets the disclosure allowlist consume a target's SECURITY-INSIGHTS.yml as self-attested evidence (ADR-033). SARIF only emitted when `--sarif-output` is set. `target_path` in the JSON / Markdown report is the target's basename by default — pass `--include-absolute-path` to keep the full absolute path. |
...
| `collect-evidence` | `--target/-t`, `--platform` (`github`/`gitlab`/`azure`/`aws`) | `--repo`, `--output-dir/-o`, `--dry-run` | `--dry-run` reports presence/absence of credential env vars without printing values, and does not require `--target` to exist on disk. GitLab uses `GITLAB_TOKEN` (+ optional `GITLAB_URL`) and `--repo group/project`. |
