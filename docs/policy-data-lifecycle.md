...
On the bundle **targeting v4.0.0** (development branch — **not yet** released or tagged), the catalog contains **65** controls; **all** are `lifecycle: stable`.

There are no `experimental` or `deprecated` controls in that catalog snapshot. Controls that were previously labelled `experimental` have been promoted to `stable`; the two YAML-heuristic controls below were carried as **deprecated** through **v3.x** and **removed** as part of the **v4.0.0** preparation.
