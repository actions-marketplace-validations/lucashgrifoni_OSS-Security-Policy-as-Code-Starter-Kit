...
def _parse_waiver_expiry(idx: int, item: dict[str, Any], today: date, warnings: list[str]) -> tuple[date | None, bool]:
...
def _parse_cdx_justification(idx: int, item: dict[str, Any], warnings: list[str]) -> str | None:
...
def _parse_vuln_waiver_entry(
    idx: int, item: Any, today: date, warnings: list[str]
) -> tuple[list[Any], _VulnWaiver] | None:
    ...
    if not owner:
        ...
        return None
    expires_at, ok = _parse_waiver_expiry(idx, item, today, warnings)
    if not ok:
        return None
    record = _VulnWaiver(
        ...
        cdx_justification=_parse_cdx_justification(idx, item, warnings),
    )
