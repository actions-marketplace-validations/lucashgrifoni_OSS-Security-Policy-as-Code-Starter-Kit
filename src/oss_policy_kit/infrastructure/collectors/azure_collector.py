...
class AzureDevOpsEvidenceCollector(EvidenceCollector):
    ...
    def collect(self, repo_slug: str) -> list[CollectionResult]:  # noqa: C901
        ...
        **Optional (manual-only) hooks:** :meth:`collect_sbom_artifact` / :meth:`collect_provenance_artifact` are
        ...
        meta = _AzCollectMeta(
            collected_at=collected_at, attested_date=attested_date, attested_by=attested_by, base=self._base
        )
        try:
            with httpx.Client(
                base_url=self._base,
                ...
                repo = _fetch_azure_repo(client, meta, project, repo_name)
                if repo is None:
                    return []
                repo_id, branch = repo
                results.append(_collect_azure_branch_policy(client, meta, project, repo_name, repo_id, branch))
                results.append(_collect_azure_governance(client, meta, project))
...
class _AzCollectMeta:
    """Shared timestamps + base URL threaded through the Azure collection helpers."""
...
def _fetch_azure_repo(client: Any, meta: _AzCollectMeta, project: str, repo_name: str) -> tuple[str, str] | None:
    """Return ``(repo_id, default_branch)`` for ``repo_name``; None on a 422 abort (raises on 401/403/404)."""
    ...
    return repo_id, _default_branch_name(repo_obj)


def _fetch_azure_policy_configs(client: Any, meta: _AzCollectMeta, project: str) -> tuple[list[dict[str, Any]], int]:
    """Return ``(policy_configurations, http_status)`` for a project (empty list on 404/422)."""
    ...
    if r_pol.status_code in {401, 403}:
        _raise_permission(f"{meta.base}{pol_url}", r_pol.status_code)
    if r_pol.status_code == 404:
        return [], r_pol.status_code
    if r_pol.status_code == 422:
        logger.warning("Azure returned 422 for %s; using empty policy list.", pol_url)
        return [], r_pol.status_code
    r_pol.raise_for_status()
    vals = cast(dict[str, Any], r_pol.json()).get("value")
    return (cast(list[dict[str, Any]], vals) if isinstance(vals, list) else []), r_pol.status_code


def _collect_azure_branch_policy(
    client: Any, meta: _AzCollectMeta, project: str, repo_name: str, repo_id: str, branch: str
) -> CollectionResult:
    """Build the azure-branch-policies evidence result from the policy-configurations API."""

    pol_url = f"/{project}/_apis/policy/configurations?api-version=7.1"
    pol_cfgs, pol_status = _fetch_azure_policy_configs(client, meta, project)
    bp_payload: dict[str, Any] = {
        ...
        "branch": branch,
        "posture": _infer_branch_posture(pol_cfgs, repo_id),
        "posture_support": {"policies_api_reachable": pol_status == 200},
        "collection": _azure_collection_block(meta.collected_at, f"{meta.base}{pol_url}"),
        "notes": "Derived from Azure DevOps policy configurations API.",
    }
    return CollectionResult(
        evidence_key="azure-branch-policies",
        data=bp_payload,
        source_url=f"{meta.base}{pol_url}",
        collected_at=meta.collected_at,
    )


def _azure_pipelines_reachable(client: Any, meta: _AzCollectMeta, project: str) -> int:
    """Poll the pipelines API solely for reachability; return its HTTP status (raises on 401/403)."""
    ...
    if r_pipe.status_code in {401, 403}:
        _raise_permission(f"{meta.base}{pipe_url}", r_pipe.status_code)
...
def _fetch_azure_environments(client: Any, project: str) -> tuple[list[dict[str, Any]], int]:
    """Return ``(environments, http_status)`` from the distributed-task environments API."""
    ...
    if r_env.status_code == 200:
        ev = cast(dict[str, Any], r_env.json()).get("value")
    ...
    return env_list, r_env.status_code


def _azure_environment_approvals(
    client: Any, project: str, env_list: list[dict[str, Any]], env_status: int
) -> tuple[bool, bool]:
    """Return ``(approval_observable, approval_found)`` by sampling environment approval checks."""
    ...
    for env in env_list[:25]:
        ...
        if sub is None:
            return False, approval_found
        if sub:
            approval_found = True
    return approval_observable, approval_found


def _collect_azure_governance(client: Any, meta: _AzCollectMeta, project: str) -> CollectionResult:
    """Build the azure-pipeline-governance evidence result (pipelines, environments, service connections)."""

    pipe_url = f"/{project}/_apis/pipelines?api-version=7.1"
    pipe_status = _azure_pipelines_reachable(client, meta, project)
    env_list, env_status = _fetch_azure_environments(client, project)
    approval_observable, approval_found = _azure_environment_approvals(client, project, env_list, env_status)
    ...
    gov_payload: dict[str, Any] = {
        ...
        "posture": {
            ...
            "environment_checks_enabled": len(env_list) > 0 and env_status == 200,
        ...
        "posture_support": {
            "pipelines_api_reachable": pipe_status == 200,
            "environments_api_reachable": env_status == 200,
            "service_endpoints_api_verified": sc_http == 200,
            "environment_approval_checks_observable": approval_observable,
        },
        "collection": _azure_collection_block(meta.collected_at, f"{meta.base}{pipe_url}"),
        "notes": (
    ...
    return CollectionResult(
        ...
        data=gov_payload,
        source_url=f"{meta.base}{pipe_url}",
        collected_at=meta.collected_at,
    )
